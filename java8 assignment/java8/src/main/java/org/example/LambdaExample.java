...
public class LambdaExample {
    ...
    public static void main(final String[] args) {
        ...
        Predicate<User> usersWithAgeGreaterThan5 = new Predicate<User>() {
            @Override
            public boolean test(User user) {
                return user.age > 5;
            }
        ...
        repository.select(usersWithAgeGreaterThan5, nameComparator);
        ...
        // TODO With functional interfaces declared
        Predicate<User> usersWithAgeLessThan10 = new Predicate<User>() {
            @Override
            public boolean test(User user) {
                return user.age < 10;
            }
        ...
        repository.select(usersWithAgeLessThan10, ageComparator);
        ...
        // TODO With functional interfaces declared
        Comparator<User> activeUserComparator = new Comparator<User>() {
            @Override
            public int compare(User u1, User u2) {
                return u1.name.compareTo(u2.name);
            }
        };
        repository.select(activeUserPredicate, activeUserComparator);
        ...
        // TODO With functional interfaces declared
        Predicate<User> activeUsersAndGreaterThan8Predicate = new Predicate<User>() {
            @Override
            public boolean test(User user) {
                return user.active && user.age > 8;
            }
        };
        repository.select(activeUsersAndGreaterThan8Predicate, nameComparator);
